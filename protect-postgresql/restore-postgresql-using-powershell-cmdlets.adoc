...
summary: 還原 PostgreSQL 備份包括啟動與 SnapCenter 伺服器的連線工作階段、列出備份和擷取備份資訊、以及還原備份。 
---
...
NOTE: AppObjectId 為「 Host\Plugin\UID 」、其中 UID = <instance_name> 用於手動探索的 PostgreSQL 執行個體資源、而 UID = <instance_name> = <databse_name> 用於 PostgreSQL 叢集資源。您可以從Get-smResources Cmdlet取得資源ID。
