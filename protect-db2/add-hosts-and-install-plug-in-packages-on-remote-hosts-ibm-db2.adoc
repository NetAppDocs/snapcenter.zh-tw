...
. 按一下*提交*。
+
如果您未選取 *Skip prechecks* 核取方塊，系統將驗證主機是否符合安裝外掛程式的要求。系統會檢查磁碟空間、RAM、PowerShell 版本、.NET 版本、位置（針對 Windows 外掛程式）以及 Java 17、Java 11（僅適用於 SnapCenter 6.2）是否符合 Windows 和 Linux 外掛程式的最低要求。如果未滿足最低要求,系統將顯示相應的錯誤或警告訊息。
