...
. 按一下*提交*。
+
如果您未選取「跳過預檢查」核取方塊，系統將驗證主機是否符合安裝外掛程式的要求。系統會根據最低要求驗證磁碟空間、RAM、PowerShell 版本、.NET 版本、位置（針對 Windows 外掛程式）以及 Java 17（針對 Windows 和 Linux 外掛程式）。如果未滿足最低要求，則會顯示相應的錯誤或警告訊息。
