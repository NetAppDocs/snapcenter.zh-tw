...
. 在「資源」頁面中，選取 *Add IBM DB2 Resource* 。
...
掛載路徑
 a| 
輸入資源掛載的掛載路徑。此操作僅適用於 Windows 主機。
...
. 選用：在「資源設定」頁面中，對於已安裝 SnapCenter Plug-in for IBM Db2 的 Linux 主機上的資源，可以輸入 IBM Db2 外掛程式的自訂鍵值對。對於註冊為集中式 Windows 外掛程式的 IBM Db2 資料庫，不支援自訂鍵值對。
. 檢閱摘要、然後按一下「*完成*」。
+
資料庫會與主機名稱、相關資源群組和原則以及整體狀態等資訊一起顯示。
