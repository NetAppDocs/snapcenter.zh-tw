...
+
如需詳細資訊、請參閱 https://["如何設定支援的SSL加密套件"]。
...
* Windows SnapCenter 和Linux主機機器的支援以支援使用支援VMware的功能ONTAP 。 https://["深入瞭解"]。
...
* 支援SnapCenter 在伺服器和ONTAP 功能區之間進行CA認證驗證。 https://["深入瞭解"]。
...
您也可以部署適用於SnapCenter VMware vSphere的VMware vSphere的VMware認證外掛程式。如需詳細資訊、請參閱 https://["建立及匯入憑證"^]。
