...
如需 ONTAP 和應用程式外掛程式的最低需求、請參閱 https://imt.netapp.com/matrix/imt.jsp?components=121030;&solution=1259&isHWU&src=IMT[] [ 互通性對照表工具 ^] 。
...
作業系統（64位元）
 a| 
請參閱 https://imt.netapp.com/matrix/imt.jsp?components=121030;&solution=1259&isHWU&src=IMT["互通性對照表工具"^]
...
* 7 GB、適用於SnapCenter 伺服器軟體和記錄
* 8 GB、用於SnapCenter 儲存庫
* 每個外掛程式主機：2 GB用於外掛程式安裝和記錄、只有在外掛程式安裝在專用主機上時才需要。



NOTE: 如果您的 SnapCenter 伺服器位於安裝 SnapCenter 儲存庫的同一主機、則建議使用 15 GB 。
