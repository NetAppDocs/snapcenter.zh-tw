...
* 您必須在 Linux 或 AIX 主機上安裝 Java 17（適用於 SnapCenter 6.2.1 及更新版本）或 Java 11（適用於 SnapCenter 6.2 及更早版本）。
+
** Linux 支援 Oracle、OpenJDK 或 SAP JDK 中的 Java。
** IBM Java for AIX。您可以從 https://developer.ibm.com/languages/java/semeru-runtimes/downloads/?version=17&os=AIX["IBM Sceru RunTimes 下載"^]下載。
+

NOTE: 請確保在 Linux 主機上僅安裝了經過認證的 Java 17 版本（適用於 SnapCenter 6.2.1 及更高版本）或 Java 11 版本（適用於 SnapCenter 6.2 及更早版本）。
...
必要的軟體套件
 a| 
* Java 17（Oracle Java、OpenJDK 或 SAP JDK）適用於 SnapCenter 6.2.1 及更新版本，Java 11 適用於 SnapCenter 6.2 及更早版本



NOTE: 請確保您在 Linux 主機上僅安裝了適用於 SnapCenter 6.2.1 及更高版本的 Java 17 認證版本或適用於 SnapCenter 6.2 及更早版本的 Java 11 認證版本。
...
* 編輯  `/etc/ssh/sshd_config` 檔案以設定訊息認證碼演算法：MACs hmac-sha2-256 和 MACs hmac-sha2-512。
+
更新組態檔後、重新啟動sshd服務。
...
必要的軟體套件
 a| 
* IBM Java 17（適用於 SnapCenter 6.2.1 及更高版本）或 IBM Java 11（適用於 SnapCenter 6.2 及更早版本）
...
CAUTION: 雖然您可以在部署主機和安裝外掛程式之後建立認證資料，但*最佳實務做法*是在新增 SVM 之後、部署主機和安裝外掛程式之前建立認證資料。
...
NOTE: 如果您在建立認證時指定不正確的詳細資料，系統會顯示錯誤訊息。您必須選取 *Cancel*，然後重試。

.步驟
. 在左側導覽窗格中、選取 * 資源 * 、然後從清單中選取適當的外掛程式。
. 在「資源」頁面中、從*檢視*清單中選取*資料庫*。
. 選擇 image:../media/filter_icon.gif["篩選圖示"]，然後選擇主機名稱和資料庫類型來篩選資源。
+
然後您可以選擇 image:../media/filter_icon.gif["篩選圖示"] 關閉篩選窗格。

. 選擇資料庫，然後選擇 *Database Settings* > *Configure Database* 。
...
+
如需詳細資訊，請參閱link:../protect-sco/task_define_a_backup_strategy_for_oracle_databases.html#preferred-nodes-in-rac-setup["RAC設定中的慣用節點"^]。
