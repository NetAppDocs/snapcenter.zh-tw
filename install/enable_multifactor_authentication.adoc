...
* 執行即可取得可搭配 Cmdlet 使用之參數的相關資訊及其說明 `Get-Help command_name`。或者、您也可以參閱 https://["《軟件指令程式參考指南》SnapCenter"^]。
...
. 從下載AD FS聯盟中繼資料檔案 "https://[] fqfq>/ 聯邦中繼資料 /2007/06/Federation中繼 資料 .xml" 。
...
.步驟
. 從下載AD FS聯盟中繼資料檔案 "https://[] Fqd>>/資料中繼資料/2007/06/FedationMetadata。XML」
